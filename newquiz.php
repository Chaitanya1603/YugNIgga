...
} else if (isset($_GET['page']) && $_GET['page'] == "create_assessment") {
    $quizDao->createAssessment($asmnt_name, $_GET['module'], '1');
} else if (isset($_GET['page']) && $_GET['page'] == "mc") {
    if (isset($_GET['question_content']) && isset($_GET['points']) && isset($_GET['correct_input']) && isset($_GET['mc_1']) && isset($_GET['mc_2']) && isset($_GET['mc_3']) && isset($_GET['mc_4'])) {
...
} else if (isset($_GET['page']) && $_GET['page'] == "fill") {
    if (isset($_GET['question_content']) && isset($_GET['points']) && isset($_GET['option_blank'])) {
        ...
        <div class="row">
          ...
          <?php
               if (isset($_GET['module'])) {
                ...
                $query = "SELECT module_name FROM students.modules WHERE id = '$moduleId';";
                ...
                // Handle case where 'module' key is not present in $_GET array
                $moduleName = "Unknown Module";
            }
            
                ?>

            <h1><i class="fa fa-graduation-cap" aria-hidden="true"></i> <?php echo $_GET['assessment_name'] ?> <br><small><?php if (isset($b)) {
    echo "<small>$b</small>";}?></small></h1>
              <hr>
            ...
            <div class="col-md-6">
                    <?php
                    $i=0;
                    $query = "select a.name, q.content, q.type, q.assessment_id, q.id  from students.questions
                    q join students.assessments a on a.id = q.assessment_id where a.name ='$asmnt_name'";
                    if ($result = $db->query($query)) {
                        /* fetch associative array */
                        while ($row = $result->fetch_assoc()) {
                            $i++;
                    ...
                    <?php
                        ...
                        $answer_query = "select answer, correct from students.answers where question_id=$row[id]";
