...
if (isset($_GET['page']) && $_GET['page'] == "mc")  {
    if (isset($_GET['question_content'])) {
        //echo $_GET['question_content'];

        $sql = "INSERT INTO students.questions (content, type, assessment_id)
    SELECT '${_GET['question_content']}','mc', assmnt.id
    from students.assessments assmnt where name= '$asmnt_name'";
        ...
        $sql_answer = "INSERT INTO students.answers (answer, correct, question_id, assessment_id)
...
if (isset($_GET['page']) && $_GET['page'] == "fill") {
    if ($_GET['question_content']) {
        ...
        $sql = "INSERT INTO students.questions (content, type, assessment_id)
    SELECT '${_GET['question_content']}','fill', assmnt.id
    from students.assessments assmnt where name= '$asmnt_name'";
        if ($db->query($sql) === TRUE) {
...
<?php require_once('include/header.php'); ?>
    ...
    <div class="row">
        ...
        <?php
            ...
            $query = "SELECT module_name FROM students.modules WHERE id = " . $_GET['module'] . ";";
...
<hr>
    ...
    $assessment_id = 0;
    $i=0;
    $query = "SELECT a.name, q.content, q.type, q.assessment_id, q.id  FROM students.questions q JOIN students.assessments a ON a.id = q.assessment_id WHERE a.name ='$asmnt_name'";
...
?>
            <div class="form-group">
                ...
                <?php
                    ...
                    $answer_query = "SELECT id, answer, correct FROM students.answers WHERE question_id=$row[id]";
                ...
                ?>
                                <div class="radio">
                                    <label>
                                        <input type="radio" name="question_<?=$row['id']?>"
                                            <?=$answer_row['correct']=='y'?"checked=\"true\"":"" ?> value="<?=$answer_row['answer']?>">
                                        <?=$answer_row['answer']?>
                                    </label>
                                </div>
                            <?php } else if ($row['type'] == "fi"){ ?>
